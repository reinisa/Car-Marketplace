...
function UploadImages({triggerUploadImages,setLoader,carInfo,mode}) {
    ...
    useEffect(()=>{
        if(triggerUploadImages){
            UploadImageToServer();
            console.log("useEffect ar triggerupload");
        }
    },[triggerUploadImages])
  ...
  return (
    <div>
        <h2 className='font-medium text-xl my-3 text-left'>Upload Car Images</h2>
        <div className='grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-5'>

            {mode=='edit'&&
                EditCarImageList.map((image,index)=>(
                    <div key={index}>
                        <IoMdCloseCircle className='absolute m-1 h-7 text-lg' 
